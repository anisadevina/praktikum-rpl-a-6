...
class MataKuliahViewModel : ViewModel() {
    ...
    fun fetchMatkul(query: String = "", page: Int = 1, token: String = "") {
        viewModelScope.launch {
            if (page == 1) _isLoading.value = true else _isPaginating.value = true
            _error.value = null
            try {
                val response = ApiClient.instance.getMatkul("Bearer $token", query, page)
    ...
    fun loadMore(token: String) {
        if (_currentPage.value < _lastPage.value && !_isLoading.value && !_isPaginating.value) {
            val nextPage = _currentPage.value + 1
            fetchMatkul(_searchQuery.value, nextPage, token)
        }
    }
}